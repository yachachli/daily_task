...
async def filter_existing_analysis_params(
    ...
    async with pool.acquire() as conn:
        existing_keys = {
            ...
            for row in await db.nfl_recent_analysis_keys(conn, days=1)
    ...
    if unresolved_count:
        logger.info(f"Skipped {unresolved_count} unresolved NFL bets before analysis")
    return filtered
...
async def run(pool: DBPool):
    # Look ahead 4 days for NFL games since DFS games are scheduled further out
