...
class NbaAltMap:
    ...
    async def from_db(cls, pool: DBPool) -> t.Self:
        [players, teams] = await asyncio.gather(
            NbaAltMap._load_nba_alt_players_from_db(pool),
            NbaAltMap._load_nba_alt_teams_from_db(pool),
        )
...
async def get_analysis_params(
    ...
    match await fetch_tomorrow_events(client, SPORT_KEY):
        case Ok(events):
            ...
        case Err() as e:
            logger.error(f"Error fetching tomorrow's NBA events: {e!r}")
            return []
...
async def run(pool: DBPool):
    ...
    nba_alt_map = await NbaAltMap.from_db(pool)
                    ...
                    for lang in languages:
                        try:
                            ...
                            if translated is not None:
                                ...
                                await conn.execute(
                                    f"UPDATE v2_nba_daily_bets SET analysis_{lang} = $1 "
