...
def resolve_player_context(
    ...
    player = nba_map.player_name_to_player_id(player_name, team_abv_home)
    if player:
        return (player.player_id, team_abv_home, team_abv_away, game_tag)

    player = nba_map.player_name_to_player_id(player_name, team_abv_away)
    if player:
...
async def filter_existing_analysis_params(
    ...
    async with pool.acquire() as conn:
        existing_keys = {
            ...
            for row in await db.nba_alt_recent_analysis_keys(conn, days=1)
            if row.stat is not None
        }
        for event, outcome, stat in params:
            resolved = resolve_player_context(nba_map, event, outcome.description)
            if not resolved:
                unresolved_count += 1
                continue
            ...
            if key in existing_keys:
                skipped_count += 1
                logger.info(
                    f"    Skipping existing NBA alt bet: {outcome.description} {stat} {outcome.point} {game_tag}"
                )
                continue
    ...
    if skipped_count:
        logger.info(f"Skipped {skipped_count} existing NBA alt bets before analysis")
    if unresolved_count:
        logger.info(
            f"Skipped {unresolved_count} unresolved NBA alt bets before analysis"
        )
    return filtered
