...
async def nba_analysis_exists(conn: ConnectionLike, *, game_time: datetime.datetime, game_tag: str, player_id: int, stat: str, line: decimal.Decimal) -> bool | None:
    row = await conn.fetchrow(NBA_ANALYSIS_EXISTS, game_time, game_tag, player_id, stat, line)
    if row is None:
        return None
    return row[0]
...
async def nba_upsert_analysis(conn: ConnectionLike, *, analysis_json: str, price: float | None, game_time: datetime.datetime, game_tag: str) -> int | None:
    row = await conn.fetchrow(NBA_UPSERT_ANALYSIS, analysis_json, price, game_time, game_tag)
    if row is None:
        return None
    return row[0]
