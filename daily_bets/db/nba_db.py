...
__all__: collections.abc.Sequence[str] = (
    ...
    "NbaRecentAnalysisKeysRow",
    "QueryResults",
    "nba_analysis_exists",
    "nba_copy_analysis",
    "nba_dedupe_recent_analysis",
    "nba_players_with_team",
    "nba_recent_analysis_keys",
    "nba_teams",
    "nba_upsert_analysis",
)
...
class NbaRecentAnalysisKeysRow(msgspec.Struct):
    game_time: datetime.datetime
    game_tag: str
    player_id: int
...
NBA_ANALYSIS_EXISTS: typing.Final[str] = """-- name: NbaAnalysisExists :one
SELECT EXISTS (
    SELECT 1
...
NBA_DEDUPE_RECENT_ANALYSIS: typing.Final[str] = """-- name: NbaDedupeRecentAnalysis :execrows
WITH ranked AS (
    SELECT
        id,
        row_number() OVER (
...
NBA_RECENT_ANALYSIS_KEYS: typing.Final[str] = """-- name: NbaRecentAnalysisKeys :many
SELECT
    game_time,
    game_tag,
...
NBA_UPSERT_ANALYSIS: typing.Final[str] = """-- name: NbaUpsertAnalysis :one
WITH inserted AS (
...
async def nba_analysis_exists(conn: ConnectionLike, *, game_time: datetime.datetime, game_tag: str, analysis: str, analysis: str, dollar_5: decimal.Decimal) -> bool | None:
    row = await conn.fetchrow(NBA_ANALYSIS_EXISTS, game_time, game_tag, analysis, analysis, dollar_5)
    if row is None:
        return None
    return row[0]
...
def nba_recent_analysis_keys(conn: ConnectionLike, *, days: int) -> QueryResults[NbaRecentAnalysisKeysRow]:
    def _decode_hook(row: asyncpg.Record) -> NbaRecentAnalysisKeysRow:
        return NbaRecentAnalysisKeysRow(game_time=row[0], game_tag=row[1], player_id=row[2], stat=row[3], line=row[4])
    return QueryResults[NbaRecentAnalysisKeysRow](conn, NBA_RECENT_ANALYSIS_KEYS, _decode_hook, days)
...
async def nba_upsert_analysis(conn: ConnectionLike, *, analysis: str, price: float | None, game_time: datetime.datetime, game_tag: str) -> int | None:
    row = await conn.fetchrow(NBA_UPSERT_ANALYSIS, analysis, price, game_time, game_tag)
    if row is None:
        return None
    return row[0]
