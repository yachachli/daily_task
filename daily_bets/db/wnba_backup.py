...
async def dedupe_backup(conn: ConnectionLike) -> None:
    """Delete duplicate rows in public.v2_wnba_daily_bets_backup keeping latest created_at.
    ...
    _ = await conn.execute(
        ...
        WITH ranked AS (
          ...
          FROM public.v2_wnba_daily_bets_backup
        )
        DELETE FROM public.v2_wnba_daily_bets_backup b
        USING ranked r
        WHERE b.ctid = r.ctid
          AND r.rn > 1;
        """
    ...
    _ = await conn.execute(
        ...
        BEGIN
          IF NOT EXISTS (
            ...
            AND    indexname = 'ux_v2_wnba_daily_bets_backup_id'
          ) THEN
            EXECUTE 'CREATE UNIQUE INDEX ux_v2_wnba_daily_bets_backup_id ON public.v2_wnba_daily_bets_backup (id)';
          END IF;
        END$$;
        """
    )
...
async def sync_recent_to_backup(conn: ConnectionLike, *, days: int = 14) -> int:
    ...
    command_tag = await conn.execute(
        """
        INSERT INTO public.v2_wnba_daily_bets_backup
        ...
        SELECT b.id, b.analysis, b.created_at, b.price, b.game_time, b.game_tag,
               b.analysis_es
        FROM public.v2_wnba_daily_bets b
        ...
        -- would leave it NULL forever -- the primary row is deleted once it
        -- ages out, and the translation would be lost with it. Fill it in on a
        -- later pass instead, and only when it is still missing, so this can
        -- never overwrite an archived translation with a newer NULL.
        ON CONFLICT (id) DO UPDATE
            SET analysis_es = EXCLUDED.analysis_es
            WHERE public.v2_wnba_daily_bets_backup.analysis_es IS NULL
