...
async def nba_alt_copy_analysis(
    conn: ConnectionLike, *, params: collections.abc.Sequence[NbaAltCopyAnalysisParams]
) -> int:
    records = [
...
async def nba_alt_analysis_exists(
    conn: ConnectionLike,
    *,
    game_time: datetime.datetime,
    game_tag: str,
    ...
    return bool(
        await conn.fetchval(
            NBA_ALT_ANALYSIS_EXISTS, game_time, game_tag, player_id, stat, line
        )
    )
...
def nba_alt_players_with_team(conn: ConnectionLike) -> QueryResults[NbaAltPlayersWithTeamRow]:
    ...
    return QueryResults[NbaAltPlayersWithTeamRow](
        conn, NBA_ALT_PLAYERS_WITH_TEAM, _decode_hook
    )
