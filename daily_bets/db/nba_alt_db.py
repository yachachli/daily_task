...
__all__: collections.abc.Sequence[str] = (
    "NbaAltCopyAnalysisParams",
    "NbaAltPlayersWithTeamRow",
    "NbaAltRecentAnalysisKeysRow",
    "QueryResults",
    "nba_alt_analysis_exists",
    "nba_alt_copy_analysis",
    "nba_alt_dedupe_recent_analysis",
    "nba_alt_players_with_team",
    "nba_alt_recent_analysis_keys",
    "nba_alt_teams",
...
class NbaAltPlayersWithTeamRow(msgspec.Struct):
    id: int
    name: str
    position: str | None
    team_id: int
...
class NbaAltRecentAnalysisKeysRow(msgspec.Struct):
    game_time: datetime.datetime
    game_tag: str
    player_id: int
...
NBA_ALT_ANALYSIS_EXISTS: typing.Final[str] = """-- name: NbaAltAnalysisExists :one
SELECT EXISTS (
    SELECT 1
...
NBA_ALT_COPY_ANALYSIS: typing.Final[str] = """-- name: NbaAltCopyAnalysis :copyfrom
INSERT INTO v2_nba_alt_daily_bets (analysis, price, game_time, game_tag) VALUES ($1, $2, $3, $4)
"""

NBA_ALT_DEDUPE_RECENT_ANALYSIS: typing.Final[str] = """-- name: NbaAltDedupeRecentAnalysis :execrows
WITH ranked AS (
    SELECT
        id,
        row_number() OVER (
...
NBA_ALT_RECENT_ANALYSIS_KEYS: typing.Final[str] = """-- name: NbaAltRecentAnalysisKeys :many
SELECT
    game_time,
    game_tag,
...
async def nba_alt_analysis_exists(conn: ConnectionLike, *, game_time: datetime.datetime, game_tag: str, analysis: str, analysis: str, dollar_5: decimal.Decimal) -> bool | None:
    row = await conn.fetchrow(NBA_ALT_ANALYSIS_EXISTS, game_time, game_tag, analysis, analysis, dollar_5)
    if row is None:
        return None
    return row[0]


async def nba_alt_copy_analysis(conn: ConnectionLike, *, params: collections.abc.Sequence[NbaAltCopyAnalysisParams]) -> int:
    records = [
        (param.analysis, param.price, param.game_time, param.game_tag)
        for param in params
    ]
    r = await conn.copy_records_to_table("v2_nba_alt_daily_bets", columns=["analysis", "price", "game_time", "game_tag"], records=records)
...
def nba_alt_players_with_team(conn: ConnectionLike) -> QueryResults[NbaAltPlayersWithTeamRow]:
    def _decode_hook(row: asyncpg.Record) -> NbaAltPlayersWithTeamRow:
        return NbaAltPlayersWithTeamRow(id=row[0], name=row[1], position=row[2], team_id=row[3], player_pic=row[4], player_id=row[5], injury=row[6], team_abv=row[7])
    return QueryResults[NbaAltPlayersWithTeamRow](conn, NBA_ALT_PLAYERS_WITH_TEAM, _decode_hook)


def nba_alt_recent_analysis_keys(conn: ConnectionLike, *, days: int) -> QueryResults[NbaAltRecentAnalysisKeysRow]:
    def _decode_hook(row: asyncpg.Record) -> NbaAltRecentAnalysisKeysRow:
        return NbaAltRecentAnalysisKeysRow(game_time=row[0], game_tag=row[1], player_id=row[2], stat=row[3], line=row[4])
    return QueryResults[NbaAltRecentAnalysisKeysRow](conn, NBA_ALT_RECENT_ANALYSIS_KEYS, _decode_hook, days)


def nba_alt_teams(conn: ConnectionLike) -> QueryResults[models.NbaTeam]:
...
async def nba_alt_upsert_analysis(conn: ConnectionLike, *, analysis: str, price: float | None, game_time: datetime.datetime, game_tag: str) -> int | None:
    row = await conn.fetchrow(NBA_ALT_UPSERT_ANALYSIS, analysis, price, game_time, game_tag)
    if row is None:
        return None
    return row[0]
