...
async def wnba_upsert_analysis(conn: ConnectionLike, *, analysis_json: str, price: float | None, game_time: datetime.datetime, game_tag: str) -> int | None:
    row = await conn.fetchrow(WNBA_UPSERT_ANALYSIS, analysis_json, price, game_time, game_tag)
    if row is None:
        return None
    return row[0]
