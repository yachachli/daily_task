...
async def nfl_analysis_exists(conn: ConnectionLike, *, game_time: datetime.datetime, game_tag: str, player_id: int, stat: str, line: decimal.Decimal) -> bool | None:
    row = await conn.fetchrow(NFL_ANALYSIS_EXISTS, game_time, game_tag, player_id, stat, line)
    if row is None:
        return None
    return row[0]
...
def nfl_teams(conn: ConnectionLike) -> QueryResults[models.V3NflTeam]:
    def _decode_hook(row: asyncpg.Record) -> models.V3NflTeam:
        return models.V3NflTeam(id=row[0], name=row[1], team_code=row[2], wins=row[3], losses=row[4], ties=row[5], points_for=row[6], points_against=row[7], total_tackles=row[8], fumbles_lost=row[9], defensive_touchdowns=row[10], fumbles_recovered=row[11], solo_tackles=row[12], defensive_interceptions=row[13], qb_hits=row[14], tackles_for_loss=row[15], pass_deflections=row[16], sacks=row[17], fumbles=row[18], passing_td_allowed=row[19], passing_yards_allowed=row[20], rushing_yards_allowed=row[21], rushing_td_allowed=row[22], new_notes=row[23], team_logo=row[24], offense_notes=row[25], schedule_2025=row[26])
    return QueryResults[models.V3NflTeam](conn, NFL_TEAMS, _decode_hook)


async def nfl_upsert_analysis(conn: ConnectionLike, *, analysis_json: str, price: float | None, game_time: datetime.datetime, game_tag: str) -> int | None:
    row = await conn.fetchrow(NFL_UPSERT_ANALYSIS, analysis_json, price, game_time, game_tag)
    if row is None:
        return None
    return row[0]
