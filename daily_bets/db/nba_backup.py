...
async def dedupe_backup(conn: ConnectionLike) -> None:
    """Delete duplicate rows in public.v2_nba_daily_bets_backup keeping latest created_at.
    ...
    await conn.execute(
        ...
        WITH ranked AS (
          ...
          FROM public.v2_nba_daily_bets_backup
        )
        DELETE FROM public.v2_nba_daily_bets_backup b
        USING ranked r
        WHERE b.ctid = r.ctid
          AND r.rn > 1;
        """
    ...
    await conn.execute(
        ...
        BEGIN
          IF NOT EXISTS (
            ...
            AND    indexname = 'ux_v2_nba_daily_bets_backup_id'
          ) THEN
            EXECUTE 'CREATE UNIQUE INDEX ux_v2_nba_daily_bets_backup_id ON public.v2_nba_daily_bets_backup (id)';
          END IF;
        END$$;
        """
    )
...
async def sync_recent_to_backup(conn: ConnectionLike, *, days: int = 14) -> int:
    ...
    command_tag = await conn.execute(
        """
        INSERT INTO public.v2_nba_daily_bets_backup (id, analysis, created_at, price, game_time, game_tag)
        SELECT b.id, b.analysis, b.created_at, b.price, b.game_time, b.game_tag
        FROM public.v2_nba_daily_bets b
